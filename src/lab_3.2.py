...
"Lab 3.2. Dibujando recorridos en el laberinto"
...
def main():
    ...
    *******************************************************************************
    ...
    """
    Tarea 1. Dibujando recorridos
    """
    #Version 1: E como lista de adyacencia"
    g = Grafo()
    arrayIndNodos = generaLaberinto(filas, columnas, semilla, pro, g)

    ans = int(input("ELEGIR BÚSQUEDA PARA DIBUJAR RECORRIDOS: [1]DFS o [2]BFS: "))

    if ans == 1:
        #Realizamos un recorrido primero en profundidad desde la primera habitacion
        #Utilizamos un parametro extra: la "profundidad de la busqueda" con un valor inicial de 1
        t0 = time.time()
        g.DFS(0, 1)
        t1 = time.time()
        print("Tiempo que tarda DFS con filas:",filas,",columnas:",columnas,",semilla:",s,"y probabilidad:",pro,"es -->",t1-t0)

    else:
        """
        Tarea 2: Recorrido en Anchura, BFS
        """
        t0 = time.time()
        g.BFS(0,1)
        t1 = time.time()
        print("Tiempo que tarda BFS con filas:",filas,",columnas:",columnas,",semilla:",s,"y probabilidad:",pro,"es -->",t1-t0)

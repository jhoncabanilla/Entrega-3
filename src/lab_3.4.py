...
"Lab 3.4. Detectando componentes conexas y 'costuras' "
...
class Nodo:
    ...
    def __init__(self, i):
        ...
        Constructor:
            ...
            > etiqueta: valor que utilizo para calcular las componentes conexas
        """
        self.id = i
        self.visitado = False
        self.vecinos = []
...
class Grafo: 
    ...
    def EtiquetandoComponentes(self, nodo, etiqueta, visitado):
        """
        Funcion en la cual marcamos con un valor cada nodo y eje, identificando a qué componente conexa pertenece. 
        · Inputs:
            > self
            > nodo: nodo actual de la busqueda
            > etiqueta: valor asociado al nodo, que identifica la componente conexa a la que pertecene
            > visitado: lista de los nodos visitados durante la deteccion de componentes conexas
        """
        if visitado[nodo] == 0:
    ...
    def ComponentesConexas(self):
        """
        Implementacion del algoritmo de deteccion de componentes conexas en grafos no dirigidos
        """
        ...
        for nodo in self.nodos:
            if visitado[nodo] == 0:
                etiq += 1
                self.EtiquetandoComponentes(nodo, etiq, visitado)
    ...
    for i in range(filas):
        for j in range(columnas):
            ...
            if g.nodos[nodoActual].visitado == True:
                matrix[i*2+1][j*2+1] = g.nodos[nodoActual].etiqueta #En ese caso, guardamos en la casilla el valor de la etiqueta del nodo
            ...
            if i < filas-1 and int(ide(array, i+1, j)) in lista_vecinos:
                ...
                if g.nodos[ int(ide(array, i+1, j)) ].visitado == True:
                    if int(ide(array, i+1, j)) in g.nodos[nodoActual].ciclo:
                        matrix[i*2+2][j*2+1] = -100 #Valor especial para indicar que cierra un ciclo
                    else:
                        matrix[i*2+2][j*2+1] = g.nodos[ int(ide(array,i,j)) ].etiqueta
                else:
                    matrix[i*2+2][j*2+1] = 0 #Pasillo no recorrido

            #Tarea 2: Detectar costuras
            #Dibujo de las 'costuras'
            else:
                if i < filas-1:
                    #En caso de que no sea un vecino y tenga una etiqueta diferente --> Pintamos ese pasillo de un valor distinto para indicar "costura"
                    if g.nodos[ int(ide(array, i+1, j)) ].etiqueta != g.nodos[nodoActual].etiqueta:
                        matrix[i*2+2][j*2+1] = 500
            ...
            if j < columnas-1 and int(ide(array, i, j+1)) in lista_vecinos: 
                ...
                if g.nodos[ int(ide(array, i, j+1)) ].visitado == True:
                    if int(ide(array, i, j+1)) in g.nodos[nodoActual].ciclo:
                        matrix[i*2+1][j*2+2] = -100
                    else:
                        matrix[i*2+1][j*2+2] = g.nodos[ int(ide(array,i,j)) ].etiqueta 
                else:
            ...
            #Dibujo de las 'costuras'
                ...
                if j < columnas-1:
                    if g.nodos[ int(ide(array, i, j+1)) ].etiqueta != g.nodos[nodoActual].etiqueta:
                        matrix[i*2+1][j*2+2] = 500
...
def main():
    ...
    *******************************************************************************
    ...
    """
    Tarea 1. Detectar componentes conexas
    """
